...
if [[ -z "`exec_container_command $container_name "which dbeaver"`" ]]; then
    install_container_package $container_name "dbeaver"
fi

exec_container_command `whoami`@$container_name "dbeaver"
