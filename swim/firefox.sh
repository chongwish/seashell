...
if [[ -z "`exec_container_command $container_name "which firefox"`" ]]; then
    install_container_package $container_name "firefox"
fi

exec_container_command `whoami`@$container_name "firefox"
