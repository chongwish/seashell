...
if [[ -z "`exec_container_command $container_name "which emacs"`" ]]; then
    install_container_package $container_name "emacs"
fi

exec_container_command `whoami`@$container_name "emacs"
