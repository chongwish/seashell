...
if [[ -z "`exec_container_command $container_name "which libreoffice"`" ]]; then
    install_container_package $container_name "libreoffice"
fi

exec_container_command `whoami`@$container_name "libreoffice"
