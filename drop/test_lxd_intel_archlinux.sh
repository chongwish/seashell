...
import Linux.Distribution.PackageManager.Archlinux[install_intel_video_driver]
...
exec_container_command $container_name 'echo -e "nameserver 8.8.8.8" >> /run/systemd/resolve/resolv.conf'
exec_container_command $container_name 'echo -e "Server = https://mirrors.tuna.tsinghua.edu.cn/archlinux/\$repo/os/\$arch" > /etc/pacman.d/mirrorlist'
...
exec_container_command $container_name "`install_intel_video_driver`"
